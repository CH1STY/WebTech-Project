...
$sql_table = "select * from appointments where doctorId = '".$_SESSION['userid']."' ORDER BY dateTime DESC";
...
if($_SERVER["REQUEST_METHOD"]=="POST")
  ...
  {
        $sql = "UPDATE `appointments` SET `status` = '3' WHERE `appointments`.`id` = ".$_POST['cancel'].";";
        mysqli_query($conn,$sql);
        header("Refresh:0");
  }
  elseif (isset($_POST['done'])) 
  {
        $sql = "UPDATE `appointments` SET `status` = '2' WHERE `appointments`.`id` = ".$_POST['done'].";";
        mysqli_query($conn,$sql);
        header("Refresh:0");
  }
                ...
                while($row = mysqli_fetch_assoc($result))
                {
                  ...
                  if($row['status']==1)
                  {
                    echo "<td><form method=\"post\"><button type=\"submit\" name=\"cancel\" value=\"".$row['id']."\">Cancel</button>
                     <button type=\"submit\" name=\"done\" value=\"".$row['id']."\">Mark As Done</button>
                    </form>                          
                          </td>";
