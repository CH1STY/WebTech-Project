...
$id = $username = $name = $dateOfBirth = $spec = $phone = $mail = '';

$sql_name= "select * from patient where ID = ".$_SESSION['userid']." ;";
...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
	if(isset($_POST['logOut']))
	{
		...
		if($flag==0){
			$sql_update = "UPDATE patient SET `name`='".$name."',`dateOfBirth`='".$dateOfBirth."',`phone`='".$phone."' WHERE ID = ".$_SESSION['userid']." ";
...
<body>
	...
	<section id="board">
		...
		<div>
			<p class="common_label">Date Of Birth</p>
			<input class="info common_inputField" type="date" name="u_dob" value="<?php echo $dateOfBirth; ?>" readonly>
		</div>
		<div>
